...
class Adb {
    ...
    async getPackageListByType(type) {
        ...
        return appsStr.split('\n').map(line => {
            const [_, apkPath, packageName] = line.match(/package:(.+)=([^\s]+)/)
    ...
    async getAppInfo(packageInfo) {
        ...
        if (iconPath && ['.png', '.webp'].includes(path.extname(iconPath))) {
            try {
                ...
                let localPath = path.resolve(utools.getPath('temp'), 'sideyvr', packageInfo.packageName, path.parse(iconPath).base)
